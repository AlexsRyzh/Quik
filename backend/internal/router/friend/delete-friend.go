...
func DeleteFriend(c echo.Context) error {
	...
	if result1.RowsAffected == 1 && result2.RowsAffected == 0 {
		database.Delete(&friendType1)

	} else if result1.RowsAffected == 1 && result2.RowsAffected == 1 {
		database.Delete(&friendType1)

		friendType2.Type = _const.FRIENDS_TYPE_SUBCRIBER
